...
From `post_hoc_inference.utils` you can import `CONFIG`, `CONSTANTS`, and `PARAMETERS` as dictionary into your code base.  
...
```python
from post_hoc_inference.utils import CONFIG, CONSTANTS, PARAMETERS
```
...
# now the "my_config" dictionary stores all information defined in `post_hoc_inference/config/base/globals.yaml` and `_/local/globals.yml`.
